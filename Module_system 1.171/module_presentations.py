...
#https://bbs.mountblade.com.cn/thread-2099117-1-1.html
#花糕滴小知识——presentation
pos_tool = [
...
##        (gt, ":total_wage", 0),
  ...
  # [osp代码] 适用于战团的战场小地图  rubik 
    ("mini_map", prsntf_read_only, 0, [
        (ti_on_presentation_load,
         [
          (set_fixed_point_multiplier, 1000),# 设定"定点"的乘数,一般是1000 (注①)
        ...
        ###############################兵力条############################################
        # create_mesh_overlay: 创建模型层对象
        # "$g_presentation_obj_1": 这是一个全局变量(可自定义别的),用以纪录现在创建的这个对象,变量名字也暗示这是第1个对象
        # "mesh_status_troop_ratio_bar": 这个对象用到的模型,需要在module_meshes.py里创建并定义,后面有定义
        # (这个模型大家很熟悉了,就是游戏选项里战斗规模的那个标尺)
        (create_mesh_overlay, "$g_presentation_obj_1", "mesh_status_troop_ratio_bar"),
        
        # 设定pos1的x坐标和y坐标(说重新设定比较合适)
        # pos1性质是锚点,也就是对象所占矩形框的左下顶点
        (position_set_x, pos1, 30),
        (position_set_y, pos1, 700),
        # 把"$g_presentation_obj_1"这个对象放到pos1的位置,也就是把"$g_presentation_obj_1"这个对象的左下顶点对齐到pos1
        (overlay_set_position, "$g_presentation_obj_1", pos1),
        ...
        # 创建模型层对象: "$g_presentation_obj_2" (红色的条)
        # "mesh_white_plane": "$g_presentation_obj_2" 用的模型,"mesh_white_plane"本已经在module_meshes.py有定义
        # 它是一个"白板",也就是一个没有着色,也没有设定大小的矩形
        # overlay_set_color: 设定对象"$g_presentation_obj_2"的颜色(0xAA1F1F),就是"白板"的颜色,相当于着色
        # overlay_set_position: 把"$g_presentation_obj_2"这个对象放到pos1的位置 (当前pos1的坐标是(35,713))
        (create_mesh_overlay, "$g_presentation_obj_2", "mesh_white_plane"),
        (overlay_set_color, "$g_presentation_obj_2", 0xAA1F1F),
        (overlay_set_position, "$g_presentation_obj_2", pos1),
        
        # 创建模型层对象: "$g_presentation_obj_3"(蓝色的条),创建和"$g_presentation_obj_2"相仿,解释略
        # 颜色当然改了,是0x1F1FAA, 对象放的位置还是pos1的位置 (当前pos1的坐标还是(35,713))
        (create_mesh_overlay, "$g_presentation_obj_3", "mesh_white_plane"),
        (overlay_set_color, "$g_presentation_obj_3", 0x1F1FAA),
        (overlay_set_position, "$g_presentation_obj_3", pos1),
     
        # 创建模型层对象: "$g_presentation_obj_4"(绿色的条),创建和"$g_presentation_obj_2"相仿,解释略
        # 颜色当然改了,是0x1FAA1F, 对象放的位置还是pos1的位置 (当前pos1的坐标还是(35,713))
        (create_mesh_overlay, "$g_presentation_obj_4", "mesh_white_plane"),
        (overlay_set_color, "$g_presentation_obj_4", 0x1FAA1F),
        (overlay_set_position, "$g_presentation_obj_4", pos1),
        

        # 创建模型层对象: "$g_presentation_obj_5"(最左边不动的游标)
        # "mesh_status_troop_ratio_bar_button": 这个对象用到的模型,需要在module_meshes.py里创建并定义,后面有定义
        # (这个模型大家很熟悉了,就是游戏选项里战斗规模的那个游标)
        # 重新设定pos1的坐标为(35,700)
        # overlay_set_position: 把"$g_presentation_obj_5"这个对象放到pos1的位置
        (create_mesh_overlay, "$g_presentation_obj_5", "mesh_status_troop_ratio_bar_button"),
        (position_set_x, pos1, 35),
        (position_set_y, pos1, 700),
        (overlay_set_position, "$g_presentation_obj_5", pos1),
        
        # 创建模型层对象: "$g_presentation_obj_6"(最右边不动的游标),创建方法同上,只是放的坐标改为(275,700)
        (create_mesh_overlay, "$g_presentation_obj_6", "mesh_status_troop_ratio_bar_button"),
        (position_set_x, pos1, 275),
        (position_set_y, pos1, 700),
        (overlay_set_position, "$g_presentation_obj_6", pos1),

        # 创建模型层对象: "$g_presentation_obj_7"(中间靠右的会随三方兵力即时游动的游标)
        # 因为位置会动,所以在这里 ti_on_presentation_load(载入presentation) 的时候就不先确定它的准确位置了,只创建它
        (create_mesh_overlay, "$g_presentation_obj_7", "mesh_status_troop_ratio_bar_button"),

        # 创建模型层对象: "$g_presentation_obj_8"(中间靠左的会随三方兵力即时游动的游标)
        # 因为位置会动,所以在这里 ti_on_presentation_load(载入presentation) 的时候就不先确定它的准确位置了,只创建它
        (create_mesh_overlay, "$g_presentation_obj_8", "mesh_status_troop_ratio_bar_button"),
         ...
         ###############################魔力条############################################
         # create_mesh_overlay: 创建模型层对象
        # "$g_presentation_obj_9": 这是一个全局变量(可自定义别的),用以纪录现在创建的这个对象,变量名字也暗示这是第1个对象
        # "mesh_status_troop_ratio_bar": 这个对象用到的模型,需要在module_meshes.py里创建并定义,后面有定义
        # (这个模型大家很熟悉了,就是游戏选项里战斗规模的那个标尺)
        (create_mesh_overlay, "$g_presentation_obj_9", "mesh_magic_value_bar"),
        
        # 设定pos1的x坐标和y坐标(说重新设定比较合适)
        # pos1性质是锚点,也就是对象所占矩形框的左下顶点
        (position_set_x, pos1, 845),
        (position_set_y, pos1, 150),
        # 把"$g_presentation_obj_9"这个对象放到pos1的位置,也就是把"$g_presentation_obj_9"这个对象的左下顶点对齐到pos1
        (overlay_set_position, "$g_presentation_obj_9", pos1),
        
        # 重新设定pos1的x坐标和y坐标
        (position_set_x, pos1, 845),
        (position_set_y, pos1, 163),
        ...
        # overlay_set_color: 设定对象"$g_presentation_obj_2"的颜色(0xAA1F1F),就是"白板"的颜色,相当于着色
        # overlay_set_position: 把"$g_presentation_obj_2"这个对象放到pos1的位置 (当前pos1的坐标是(35,713))
        (create_mesh_overlay, "$g_presentation_obj_10", "mesh_white_plane"),
        (overlay_set_color, "$g_presentation_obj_10", 0x000000),
        (overlay_set_position, "$g_presentation_obj_10", pos1),
        
        # 创建模型层对象: "$g_presentation_obj_4"(绿色的条),创建和"$g_presentation_obj_2"相仿,解释略
        # 颜色当然改了,是0x1FAA1F, 对象放的位置还是pos1的位置 (当前pos1的坐标还是(35,713))
        (create_mesh_overlay, "$g_presentation_obj_12", "mesh_white_plane"),
        (overlay_set_color, "$g_presentation_obj_12", 0x00BFFF),
        (overlay_set_position, "$g_presentation_obj_12", pos1),
        
        

        # 创建模型层对象: "$g_presentation_obj_13"(最左边不动的游标)
        # "mesh_status_troop_ratio_bar_button": 这个对象用到的模型,需要在module_meshes.py里创建并定义,后面有定义
        # (这个模型大家很熟悉了,就是游戏选项里战斗规模的那个游标)
        # 重新设定pos1的坐标为(35,700)
        # overlay_set_position: 把"$g_presentation_obj_13"这个对象放到pos1的位置
        (create_mesh_overlay, "$g_presentation_obj_13", "mesh_magic_value_button"),
        (position_set_x, pos1, 845),
        (position_set_y, pos1, 157),
        (overlay_set_position, "$g_presentation_obj_13", pos1),
        
        # 创建模型层对象: "$g_presentation_obj_14"(最右边不动的游标),创建方法同上,只是放的坐标改为(275,700)
        (create_mesh_overlay, "$g_presentation_obj_14", "mesh_magic_value_button"),
        (position_set_x, pos1, 967),
        (position_set_y, pos1, 157),
        (overlay_set_position, "$g_presentation_obj_14", pos1),


        # 创建模型层对象: "$g_presentation_obj_16"(中间靠左的会随三方兵力即时游动的游标)
        # 因为位置会动,所以在这里 ti_on_presentation_load(载入presentation) 的时候就不先确定它的准确位置了,只创建它
        (create_mesh_overlay, "$g_presentation_obj_16", "mesh_magic_value_button"),
...
########### 计算当前agent的实力 ###########
          (try_begin),
            ...
            (val_add, ":player_count", ":level"),
          (else_try),
            (agent_is_ally, ":cur_agent"),
            (val_add, ":ally_count", ":level"),
          (else_try),
            (val_add, ":enemy_count", ":level"),
          (try_end),
        (try_end),
...
############## 确定3个颜色条的大小 #############
        ...
        (position_set_x, pos1, 12000),
        (position_set_y, pos1, 300),
          # 设定对象"$g_presentation_obj_2"(红色的条)的大小,在设定大小的时候,x坐标和y坐标分别代表宽和高
        (overlay_set_size, "$g_presentation_obj_2", pos1),
     
          # 把":ally_count"和":player_count"加起来再赋值到":ally_percent" (将用以纪录蓝色条的宽度)
        (store_add, ":ally_percent", ":player_count", ":ally_count"),
        (val_mul, ":ally_percent", 12000),            # ":ally_percent"乘以12000
        (val_div, ":ally_percent", ":total_count"),   # ":ally_percent"除以":total_count"
        (position_set_x, pos1, ":ally_percent"),      # 设定po1的x值为":ally_percent"
        (position_set_y, pos1, 300),                  # 设定po1的y值为300
          # 设定对象"$g_presentation_obj_3"(蓝色的条)的大小,宽和高分别为当前pos1的x值和y值
        (overlay_set_size, "$g_presentation_obj_3", pos1),
     
          # 把":player_count"乘以12000再赋值到":player_percent" (将用以纪录绿色条的宽度)
        (store_mul, ":player_percent", ":player_count", 12000),
        (val_div, ":player_percent", ":total_count"), # ":player_percent"除以":total_count"
        (position_set_x, pos1, ":player_percent"),    # 设定po1的x值为":player_percent"
        (position_set_y, pos1, 300),                  # 设定po1的y值为300
          # 设定对象"$g_presentation_obj_4"(绿色的条)的大小,宽和高分别为当前pos1的x值和y值
...
############## 确定中间2个可动游标的位置 #############
          ...
          # 把":ally_count"和":player_count"加起来再赋值到":ally_percent_2"
        (store_add, ":ally_percent_2", ":player_count", ":ally_count"),
        (val_mul, ":ally_percent_2", 240),              # ":ally_percent_2"乘以240
        (val_div, ":ally_percent_2", ":total_count"),   # ":ally_percent_2"除以":total_count"
        (val_add, ":ally_percent_2", 35),               # ":ally_percent_2"加上35
        (position_set_x, pos1, ":ally_percent_2"),      # 设定pos1的x坐标为":ally_percent_2"
        (position_set_y, pos1, 700),                    # 设定pos1的y坐标为700
          # 把"$g_presentation_obj_7"(中间靠右的会动的游标)这个对象放到pos1的位置
        (overlay_set_position, "$g_presentation_obj_7", pos1),
         
          # 把":player_count"乘以240再赋值到":player_percent_2"
        (store_mul, ":player_percent_2", ":player_count", 240),
        (val_div, ":player_percent_2", ":total_count"), # ":player_percent_2"除以":total_count"
        (val_add, ":player_percent_2", 35),             # ":player_percent_2"加上35
        (position_set_x, pos1, ":player_percent_2"),    # 设定pos1的x坐标为":player_percent_2"
        (position_set_y, pos1, 700),                    # 设定pos1的y坐标为700
          # 把"$g_presentation_obj_8"(中间靠左的会动的游标)这个对象放到pos1的位置
...
############## 确定中间2个可动游标的位置 #############   
        ...
        (assign, ":player_count", 0),  # 创建局部变量":player_count",并赋值为0 (将用以纪录玩家一方的人数,当前活着的)
        (assign, ":total_count", 0),   # 创建局部变量":total_count", 并赋值为0 (将用以纪录玩所有的人数,当前活着的)
        
        (assign, ":player_count", "$g_magic_cnt"),
        (store_attribute_level, ":int", "trp_player", ca_intelligence),
        (store_mul,":total_count",":int",3),
        ...
        # 把":player_count"乘以12000再赋值到":player_percent" (将用以纪录绿色条的宽度)
        (store_mul, ":player_percent", ":player_count", 6000),
        (val_div, ":player_percent", ":total_count"), # ":player_percent"除以":total_count"
        (position_set_x, pos1, ":player_percent"),    # 设定po1的x值为":player_percent"
        (position_set_y, pos1, 300),                  # 设定po1的y值为300
          # 设定对象"$g_presentation_obj_4"(绿色的条)的大小,宽和高分别为当前pos1的x值和y值
...
############## 确定3个颜色条的大小 #############
          ...
          # 把":player_count"乘以240再赋值到":player_percent_2"
        (store_mul, ":player_percent_2", ":player_count", 120),
        (val_div, ":player_percent_2", ":total_count"), # ":player_percent_2"除以":total_count"
        (val_add, ":player_percent_2", 845),             # ":player_percent_2"加上35
        (position_set_x, pos1, ":player_percent_2"),    # 设定pos1的x坐标为":player_percent_2"
        (position_set_y, pos1, 157),                    # 设定pos1的y坐标为700
          # 把"$g_presentation_obj_8"(中间靠左的会动的游标)这个对象放到pos1的位置
        (overlay_set_position, "$g_presentation_obj_16", pos1),

############## 确定中间2个可动游标的位置 #############  
        (try_begin), # 开始try
          # 当gk_view_orders键(就是打开指挥界面的那个键)被按下
          # gk_view_orders键并不是固定的某个键,当玩家重新定义打开指挥界面的键的时候,新定义的键值会重新赋给gk_view_orders键
          # 所以gk_view_orders键的功能是固定的,不会随玩家重新定义按键而改变,也就意味着玩家可以自定义按键,而不影响这句代码的判断
          (game_key_clicked, gk_view_orders),

          (presentation_set_duration, 0), # 设定presentation运行时间为0,也就是立即结束
          (start_presentation, "prsnt_battle"), # 开始运行presentation: "prsnt_battle"(就是指挥界面,不准确地说法就是小地图)
        (try_end),  # 结束try
       ]),
    ...
    (ti_on_presentation_load,
      [
        ...
        ## get the troop
        (create_game_button_overlay, "$g_presentation_obj_admin_panel_7", "@CHEAT: get the troop"),
        (position_set_x, pos1, 675),
        ...
        (try_begin),
          (neg|troop_is_hero, ":selected_troop"),
          (troop_get_upgrade_troop, ":upgrade_troop", ":selected_troop", 0),
          (gt, ":upgrade_troop", 0),
          (call_script, "script_game_get_upgrade_xp", ":selected_troop"),
          (call_script, "script_game_get_upgrade_cost", ":selected_troop"),
          (str_store_string, s1, "@{s1}^Upgrade xp: {reg10}^Upgrade cost: {reg11}"),
          #(str_store_string, s1, "@{s1}^New Upgrade xp: {reg10}^Old Upgrade xp: {reg5}"),
        (try_end),
        ...
        (assign, ":base_y", 620),
        # background
        (create_mesh_overlay, reg1, "mesh_white_triangle_combined"),
        (position_set_x, pos1, ":base_x"),
        (position_set_y, pos1, ":base_y"),
        (overlay_set_position, reg1, pos1),
        (position_set_x, pos1, 63*50),
        (position_set_y, pos1, 63*50),
        (overlay_set_size, reg1, pos1),
        (overlay_set_color, reg1, 0),
        (overlay_set_alpha, reg1, 0x22),
        # crossline
        (create_mesh_overlay, reg1, "mesh_white_plane"),
        (store_add, ":line_x", ":base_x", -63),
        (store_add, ":line_y", ":base_y", -1),
        (position_set_x, pos1, ":line_x"),
        (position_set_y, pos1, ":line_y"),
        (overlay_set_position, reg1, pos1),
        (position_set_x, pos1, 126*50),
        (position_set_y, pos1, 50),
        (overlay_set_size, reg1, pos1),
        (overlay_set_color, reg1, 0),
        (overlay_set_alpha, reg1, 0x33),
        (create_mesh_overlay, reg1, "mesh_white_plane"),
        (store_add, ":line_x", ":base_x", -1),
        (store_add, ":line_y", ":base_y", -63),
        (position_set_x, pos1, ":line_x"),
        (position_set_y, pos1, ":line_y"),
        (overlay_set_position, reg1, pos1),
        (position_set_x, pos1, 50),
        (position_set_y, pos1, 126*50),
        (overlay_set_size, reg1, pos1),
        (overlay_set_color, reg1, 0),
        (overlay_set_alpha, reg1, 0x33),
          
        (try_for_range, ":cur_attribute_no", 0, 4),
          (store_add, ":next_attribute_no", ":cur_attribute_no", 1),
          (val_mod, ":next_attribute_no", 4),
          (store_attribute_level, ":attribute_level", ":selected_troop", ":cur_attribute_no"),
          (store_attribute_level, ":next_attribute_level", ":selected_troop", ":next_attribute_no"),
          # triangle mesh
          (store_add, ":cur_mesh", "mesh_white_triangle_1", ":cur_attribute_no"),
          (create_mesh_overlay, reg1, ":cur_mesh"),
          (position_set_x, pos1, ":base_x"),
          (position_set_y, pos1, ":base_y"),
          (overlay_set_position, reg1, pos1),
          # size
          (store_mul, ":cur_attribute_size", ":attribute_level", 50),
          (store_mul, ":next_attribute_size", ":next_attribute_level", 50),
          (try_begin),
            (this_or_next|eq, ":cur_attribute_no", 0),
            (eq, ":cur_attribute_no", 2),
            (position_set_y, pos1, ":cur_attribute_size"),
            (position_set_x, pos1, ":next_attribute_size"),
          (else_try),
            (this_or_next|eq, ":cur_attribute_no", 1),
            (eq, ":cur_attribute_no", 3),
            (position_set_x, pos1, ":cur_attribute_size"),
            (position_set_y, pos1, ":next_attribute_size"),
          (try_end),
          (position_set_x, pos2, 1),
          (position_set_y, pos2, 1),
          (overlay_set_size, reg1, pos2),
          (overlay_animate_to_size, reg1, 300, pos1),
          (overlay_set_color, reg1, 0x000033),
          # attribute text
          (assign, reg0, ":attribute_level"),
          (store_add, ":dest_text", "str_attribute_str", ":cur_attribute_no"),
          (str_store_string, s1, ":dest_text"),
          (create_text_overlay, reg1, "@{s1}^({reg0}/63)", tf_center_justify|tf_vertical_align_center),
          (try_begin),
            (eq, ":cur_attribute_no", 0),
            (assign, ":text_x", ":base_x"),
            (store_add, ":text_y", ":base_y", 8+63),
          (else_try),
            (eq, ":cur_attribute_no", 1),
            (store_add, ":text_x", ":base_x", 63),
            (store_add, ":text_y", ":base_y", 8),
          (else_try),
            (eq, ":cur_attribute_no", 2),
            (assign, ":text_x", ":base_x"),
            (store_add, ":text_y", ":base_y", 8-63),
        ...
        (try_for_range_backwards, ":skill_no", skl_horse_archery, skl_reserved_18), # 8 skills
          (this_or_next|eq, ":skill_no", skl_power_magic),
        ...
        (try_for_range, ":proficiency_no", 0, 7), 
          # icon and level
          (store_add, ":dest_icon", "mesh_fight_icon_swordone", ":proficiency_no"),
          (create_mesh_overlay, reg1, ":dest_icon"),
          (store_add, ":icon_x", ":pos_x", 20),
          (store_add, ":icon_y", ":pos_y", 12),
          ...
          (store_add, ":text_x", ":pos_x", 50),
          (store_add, ":text_y", ":pos_y", 10),
          (create_text_overlay, reg1, "@({reg0}/699)", tf_left_align),
          (position_set_x, pos1, ":text_x"),
          (position_set_y, pos1, ":text_y"),
          (overlay_set_position, reg1, pos1),
          (position_set_x, pos1, 600),
    ...
    (ti_on_presentation_event_state_change,
      [
        ...
        (else_try),
          (eq, ":object", "$g_presentation_obj_admin_panel_7"),
          (party_add_members, "p_main_party", ":selected_troop", 20),
          (str_store_troop_name_plural, s1, ":selected_troop"),
          (display_message, "@CHEAT! You get 20 {s1}.", 0xFF0000),
        (else_try),
          (eq, ":object", "$g_presentation_obj_admin_panel_1"),
          (try_begin),
...
###################### 
    # INVASION MODE END 模组选项功能设置 霸主之路宇宙人
    ### bore MOD    #ccc
    ("camp_mod_opition", 0, mesh_load_window, [
        (ti_on_presentation_load,
         [
             (set_fixed_point_multiplier, 1000),          
             ...
             (position_set_y, pos1, 600),
             (position_set_y, pos2, 600),
             (create_text_overlay, reg0, "str_mod_option1", tf_center_justify),
             (position_set_x, pos1, 300),
             (position_set_x, pos2, 450),
             (overlay_set_position, reg0, pos1),
             (create_check_box_overlay, "$npc_clash_stop_overlay", "mesh_checkbox_off", "mesh_checkbox_on"),
             (overlay_set_position, "$npc_clash_stop_overlay", pos2),
             (overlay_set_val, "$npc_clash_stop_overlay", "$g_NPC_clash_stop"),

             #骑射AI
             (create_text_overlay, reg0, "str_mod_option2", tf_center_justify),  
             (position_set_x, pos1, 600),
             (position_set_x, pos2, 750),
             (overlay_set_position, reg0, pos1),
             (create_check_box_overlay, "$mounted_shooting_AI_overlay", "mesh_checkbox_off", "mesh_checkbox_on"),
             (overlay_set_position, "$mounted_shooting_AI_overlay", pos2),
             (overlay_set_val, "$mounted_shooting_AI_overlay", "$g_mounted_shooting_AI"),

             #骑兵返回冲锋AI
             (position_set_y, pos1, 560),
             (position_set_y, pos2, 560),
             (position_set_x, pos1, 300),
             (position_set_x, pos2, 450),
             (create_text_overlay, reg0, "str_mod_option3", tf_center_justify), 
             (overlay_set_position, reg0, pos1),
             (create_check_box_overlay, "$cavalry_returns_to_charge_AI_overlay", "mesh_checkbox_off", "mesh_checkbox_on"),
             (overlay_set_position, "$cavalry_returns_to_charge_AI_overlay", pos2),
             (overlay_set_val, "$cavalry_returns_to_charge_AI_overlay", "$g_cavalry_returns_to_charge_AI"),

             #允许飞行与飞行AI
             (create_text_overlay, reg0, "str_mod_option4", tf_center_justify),
             (position_set_x, pos1, 600),
             (position_set_x, pos2, 750),
             (overlay_set_position, reg0, pos1),
             (create_check_box_overlay, "$allow_flight_with_flying_AI_overlay", "mesh_checkbox_off", "mesh_checkbox_on"),
             (overlay_set_position, "$allow_flight_with_flying_AI_overlay", pos2),
             (overlay_set_val, "$allow_flight_with_flying_AI_overlay", "$g_allow_flight_with_flying_AI"),
             
             #允许击飞
             (position_set_y, pos1, 520),
             (position_set_y, pos2, 520),
             (create_text_overlay, reg0, "str_mod_option5", tf_center_justify), 
             (position_set_x, pos1, 300),
             (position_set_x, pos2, 450),
             (overlay_set_position, reg0, pos1),
             (create_check_box_overlay, "$strike_allowed_overlay", "mesh_checkbox_off", "mesh_checkbox_on"),
             (overlay_set_position, "$strike_allowed_overlay", pos2),
             (overlay_set_val, "$strike_allowed_overlay", "$g_strike_allowed"),
             
             #友方小兵避让玩家
             (create_text_overlay, reg0, "str_mod_option6", tf_center_justify),  
             (position_set_x, pos1, 600),
             (position_set_x, pos2, 750),
             (overlay_set_position, reg0, pos1),
             (create_check_box_overlay, "$agent_avoidance_AI_overlay", "mesh_checkbox_off", "mesh_checkbox_on"),
             (overlay_set_position, "$agent_avoidance_AI_overlay", pos2),
             (overlay_set_val, "$agent_avoidance_AI_overlay", "$g_agent_avoidance_AI"),


             (create_image_button_overlay, "$g_close_story", "mesh_drop_button","mesh_drop_button_down"),
             (position_set_x, pos1, 403),
             (position_set_y, pos1, 41),
             (overlay_set_position, "$g_close_story", pos1),
             (position_set_x, pos1, 600),
             (position_set_y, pos1, 600),
             (overlay_set_size, "$g_close_story", pos1),
             (create_text_overlay, ":go_back", "@go back", tf_center_justify),
             (position_set_x, pos1, 466),
             (position_set_y, pos1, 45),
             (overlay_set_position, ":go_back", pos1),  

             (presentation_set_duration, 999999),
             
             (create_text_overlay, reg0, "str_mod_option", tf_center_justify),  # 标题
             (position_set_x, pos1, 500),
             (position_set_y, pos1, 650),
             (overlay_set_position, reg0, pos1),   
        ...
        (ti_on_presentation_event_state_change,
         [
             ...
             (try_begin),
             (eq, ":object", "$g_close_story"),
             (presentation_set_duration, 0),
             
             (else_try),
              (eq, ":object", "$npc_clash_stop_overlay"),  #NPC离队
              (assign, "$g_NPC_clash_stop", ":value"),
             (else_try),
              (eq, ":object", "$mounted_shooting_AI_overlay"), #骑射AI
              (assign, "$g_mounted_shooting_AI", ":value"),  
             (else_try),
              (eq, ":object", "$cavalry_returns_to_charge_AI_overlay"), #骑兵返回冲锋AI
              (assign, "$g_cavalry_returns_to_charge_AI", ":value"),  
             (else_try),
              (eq, ":object", "$allow_flight_with_flying_AI_overlay"), #允许飞行与飞行AI
              (assign, "$g_allow_flight_with_flying_AI", ":value"),  
             (else_try),
              (eq, ":object", "$strike_allowed_overlay"), #允许击飞
              (assign, "$g_strike_allowed", ":value"),  
             (else_try),
              (eq, ":object", "$agent_avoidance_AI_overlay"), #友方小兵避让玩家
              (assign, "$g_agent_avoidance_AI", ":value"),  
             (try_end),
         ]),
...
("all_items", 0, 0, [
    (ti_on_presentation_load,
      [
        ...
        (position_set_y, pos1, 650),
        (overlay_set_position, "$g_presentation_obj_admin_panel_1", pos1),
        (overlay_add_item, "$g_presentation_obj_admin_panel_1", "@Melee Weapons"),#0
        (overlay_add_item, "$g_presentation_obj_admin_panel_1", "@Range Weapons"),#1
        (overlay_add_item, "$g_presentation_obj_admin_panel_1", "@Armors"),#2
        (overlay_add_item, "$g_presentation_obj_admin_panel_1", "@Horses"),#3
        (overlay_add_item, "$g_presentation_obj_admin_panel_1", "@Books"),#4
        (overlay_add_item, "$g_presentation_obj_admin_panel_1", "@Goods"),#5
        (overlay_add_item, "$g_presentation_obj_admin_panel_1", "@Shields"),#6
        (overlay_set_val, "$g_presentation_obj_admin_panel_1", ":cur_page"),

        ## back
        (create_game_button_overlay, "$g_presentation_obj_admin_panel_5", "str_quit"),
        ...
        (overlay_set_position, reg1, pos1),
        ## cheat: get the item
        (create_game_button_overlay, "$g_presentation_obj_admin_panel_7", "@CHEAT: get the item"),
        (position_set_x, pos1, 835),
        (position_set_y, pos1, 200),
        (overlay_set_position, "$g_presentation_obj_admin_panel_7", pos1),
        (overlay_set_display, "$g_presentation_obj_admin_panel_7", 0),
    ...
    (ti_on_presentation_event_state_change,
      [
        ...
        (try_for_range, ":slot_no", 0, ":upper_bound"),
          ...
          (try_begin),
            (ge, "$cheat_mode", 1),
            (overlay_set_display, "$g_presentation_obj_admin_panel_7", 1),
          (try_end),
        (try_end),
        ...
        (else_try),
          (eq, ":object", "$g_presentation_obj_admin_panel_7"),
          (troop_get_slot, ":cur_item_no", "trp_temp_array_c", 0),
          (troop_add_item, "trp_player", ":cur_item_no"),
        (else_try),
          (eq, ":object", "$g_presentation_obj_admin_panel_5"),
          (presentation_set_duration, 0),
        (try_end),
...
("necromancer_redeem_items", 0, 0, [
    (ti_on_presentation_load,
      [
        ...
        (assign, ":cur_page", "$g_prsnt_param_one"),
        ...
        (position_set_y, pos1, 265),
        (overlay_set_position, reg1, pos1),
        (troop_set_slot, "trp_temp_array_l", 0, reg1),
        ## item
        (create_mesh_overlay_with_item_id, reg1, 0),
        (position_set_x, pos1, 2800),
        (position_set_y, pos1, 2800),
        ...
        (position_set_y, pos1, 265+140),
        (overlay_set_position, reg1, pos1),
        (troop_set_slot, "trp_temp_array_m", 0, reg1),
        (troop_set_slot, "trp_temp_array_n", 0, 0),
        ## txt
        (create_text_overlay, reg1, "@Preview: ", tf_left_align),
        (position_set_x, pos1, 700),
        (position_set_y, pos1, 570),
        ...
        (assign,reg10,0),
        (create_text_overlay, reg1, "str_expend", tf_left_align),
        (position_set_x, pos1, 750),
        (position_set_y, pos1, 160),
        (overlay_set_position, reg1, pos1),
        (troop_set_slot, "trp_temp_array_o", 0, reg1),
        
        (create_text_overlay, reg1, "str_own", tf_left_align),
        (position_set_x, pos1, 750),
        (position_set_y, pos1, 130),
        (overlay_set_position, reg1, pos1),
        (troop_set_slot, "trp_temp_array_p", 0, reg1),
        
        (create_text_overlay, reg1, "str_remaining", tf_left_align),
        (position_set_x, pos1, 750),
        (position_set_y, pos1, 100),
        (overlay_set_position, reg1, pos1),
        (troop_set_slot, "trp_temp_array_q", 0, reg1),
        ## BEG: get the item
        (create_game_button_overlay, "$g_presentation_obj_admin_panel_7", "@BEG: get the item"),
        (position_set_x, pos1, 835),
        (position_set_y, pos1, 200),
        (overlay_set_position, "$g_presentation_obj_admin_panel_7", pos1),
        (overlay_set_display, "$g_presentation_obj_admin_panel_7", 1),
        ...
        (try_begin),
          (eq, ":cur_page", 0), # melee weapons
          (try_for_range, ":item_no", 1, ":num_items"),
            (call_script,"script_cf_item_only_kingdom",":item_no",fac_kingdom_undead),
            (item_get_type, ":type", ":item_no"),
            (is_between, ":type", itp_type_one_handed_wpn, itp_type_arrows),
            (val_add, ":total_items", 1),
          (try_end),
        ...
        (else_try),
          ...
          (try_end),
        (try_end),
        (assign, "$g_prsnt_param_three", ":total_items"),
        ...
        (try_for_range, ":item_no", 1, ":num_items"),
          ...
          (position_set_y, pos1, ":pos_y"),
          (overlay_set_position, reg1, pos1),
          (troop_set_slot, "trp_temp_array_l", ":slot_no", reg1),
          ## item
          (create_mesh_overlay_with_item_id, reg1, ":item_no"),
          (position_set_x, pos1, 800),
          (position_set_y, pos1, 800),
          ...
          (position_set_y, pos1, ":item_y"),
          (overlay_set_position, reg1, pos1),
          (troop_set_slot, "trp_temp_array_m", ":slot_no", reg1),
          (troop_set_slot, "trp_temp_array_n", ":slot_no", ":item_no"),
          (val_add, ":pos_x", 80),
          (val_add, ":slot_no", 1),
          (try_begin),
            (ge, ":pos_x", 640),
   ...
   # ]),
    ...
    (ti_on_presentation_mouse_enter_leave,
      ...
      (assign, ":total_items", "$g_prsnt_param_three"),
      (store_add, ":upper_bound", ":total_items", 1),
      (try_begin),
        (eq, ":enter_leave", 0),
        (try_for_range, ":slot_no", 0, ":upper_bound"),
          (troop_slot_eq, "trp_temp_array_l", ":slot_no", ":object"),
          (troop_get_slot, ":item_no", "trp_temp_array_n", ":slot_no"),
          (neq, ":item_no", 0),
          (troop_get_slot, ":target_obj", "trp_temp_array_m", ":slot_no"),
          (overlay_get_position, pos0, ":target_obj"),
          (show_item_details, ":item_no", pos0, 100),
          (assign, "$g_current_opened_item_details_one", ":slot_no"),
          
          (item_get_value,":item_value",":item_no"),
          (val_div,":item_value",beg_scale),
          (assign,reg10,":item_value"),
          (troop_get_slot, ":target_obj", "trp_temp_array_o", 0),
          (overlay_set_text,":target_obj","str_expend"),
          
          (assign,reg10,"$g_necromancer_act_cnt"),
          (troop_get_slot, ":target_obj", "trp_temp_array_p", 0),
          (overlay_set_text,":target_obj","str_own"),
          
          (store_sub,":remaining","$g_necromancer_act_cnt",":item_value"),
          (assign,reg10,":remaining"),
          (troop_get_slot, ":target_obj", "trp_temp_array_q", 0),
          (overlay_set_text,":target_obj","str_remaining"),
      ...
      (else_try),
        (try_for_range, ":slot_no", 0, ":upper_bound"),
          (troop_slot_eq, "trp_temp_array_l", ":slot_no", ":object"),
          (try_begin),
            (eq, "$g_current_opened_item_details_one", ":slot_no"),
            (close_item_details),
          (try_end),
        (try_end),
      (try_end),
    ...
    (ti_on_presentation_event_state_change,
      [
        ...
        (try_for_range, ":slot_no", 0, ":upper_bound"),
          ...
          (troop_slot_eq, "trp_temp_array_l", ":slot_no", ":object"),
          (troop_get_slot, ":item_no", "trp_temp_array_n", ":slot_no"),
          (troop_get_slot, ":target_obj", "trp_temp_array_m", 0),
          (overlay_set_display, ":target_obj", 0),
          ## new item overlay
          (create_mesh_overlay_with_item_id, reg1, ":item_no"),
          (position_set_x, pos1, 2800),
          ...
          (overlay_set_position, reg1, pos1),
          (troop_set_slot, "trp_temp_array_m", 0, reg1),
          (troop_set_slot, "trp_temp_array_n", 0, ":item_no"),
          (try_begin),
            #(ge, "$cheat_mode", 1),
            (overlay_set_display, "$g_presentation_obj_admin_panel_7", 1),
          (try_end),
        ...
        (try_begin),
          (eq, ":object", "$g_presentation_obj_admin_panel_1"),
          (assign, ":cur_page", ":value"),
          (assign, "$g_prsnt_param_one", ":cur_page"),
          (start_presentation, "prsnt_necromancer_redeem_items"),
        (else_try),
          (eq, ":object", "$g_presentation_obj_admin_panel_7"),
          (troop_get_slot, ":cur_item_no", "trp_temp_array_n", 0),
          (store_free_inventory_capacity,":free_num","trp_player"),
          
          (item_get_value,":item_value",":cur_item_no"),
          (str_store_item_name,s10,":cur_item_no"),
          (val_div,":item_value",beg_scale),
          (store_sub,":remaining","$g_necromancer_act_cnt",":item_value"),
          
          (try_begin),
            (le,":free_num",0),
            (display_message, "@Player free inventory is zero",color_code_bad),
          (else_try), 
            (lt,":remaining",0),
            (display_message, "@Player not enough points",color_code_bad),
          (else_try),
            (troop_add_item, "trp_player", ":cur_item_no"),
            (val_sub,"$g_necromancer_act_cnt",":item_value"),
            (display_message, "@Beg complete {s10}",color_code_purple),
            (play_sound,"snd_undead_magic"),  
          (try_end),
        (else_try),
          (eq, ":object", "$g_presentation_obj_admin_panel_5"),
          (presentation_set_duration, 0),
